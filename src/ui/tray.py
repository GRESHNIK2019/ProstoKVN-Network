...
class TrayController:
    """Системный трей Windows через pystray без обращения к Tk из tray thread."""
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            menu = pystray.Menu(
                pystray.MenuItem("Открыть ProstoKVN Network", self._request_show, default=True),
    ...
    def minimize(self) -> bool:
        # Окно скрывается только ПОСЛЕ подтверждённого создания tray icon.
        if not self.start():
    ...
    def _tray_failed_fallback(self) -> None:
        """При ошибке трея приложение никогда не исчезает без способа выйти."""
        self.restore()
        try:
    ...
    def _fallback_icon_image() -> Any:
        """Иконка в памяти: tray не зависит от пути PyInstaller _MEI."""
    ...
    def _load_icon_image() -> Any:
        ...
        if meipass:
            candidates.extend([
                Path(meipass) / "assets" / "ProstoKVNNetwork.ico",
                Path(meipass) / "ProstoKVNNetwork.ico",
            ])
